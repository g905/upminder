    ...
    public function form($id, Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            for ($a = 0; $a < sizeof($request->input('service_service')); $a++) {
                ...
                $this_currency = array_key_exists($a, $request->get('service_currency_id'))
                    ? $request->get('service_currency_id')[$a]
                    : 0;
                $this_price = $request->input('service_price')[$a] ?? 0;
                $this_discount = array_key_exists($a, $request->get('service_discount'))
                    ? $request->get('service_discount')[$a]
                    : null;
			...
			if ($request->input('additional_service_service')) {
				for ($a = 0; $a < sizeof($request->input('additional_service_service')); $a++) {
					...
					$this_currency = array_key_exists($a, $request->get('additional_service_currency_id'))
						? $request->get('additional_service_currency_id')[$a]
						: 0;
					$this_price = $request->input('additional_service_price')[$a] ?? 0;
					$this_discount = array_key_exists($a, $request->get('additional_service_discount'))
						? $request->get('additional_service_discount')[$a]
						: null;
