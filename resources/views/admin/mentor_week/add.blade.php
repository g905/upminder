...
@section('content')
  ...
  <div class="row">
    <div class="col-md-12 grid-margin">
      ...
      <div class="card">
        <div class="card-body">
          <form action="{{ route('admin.mentor_week.store') }}" method="POST">
            @csrf
                  ...
                  @foreach ($categories as $parent)
                    <optgroup label="{{ $parent->name }}">
                      @foreach($parent->children as $category)
                        <option value="{{ $category->id }}" @if ($category->id == old('category_id', null)) selected @endif>{{ $category->name }} </option>
                      @endforeach
                    </optgroup>
                  @endforeach
                @endif
            ...
            <div class="form-group">
              ...
              <div class="form-check">
                <label class="form-check-label">
                  <input id="verified0" type="radio" name="is_active" value="0" class="form-check-input" @if(!old('is_active')) checked @endif/>
                  Не активный
              ...
              <div class="form-check">
                <label class="form-check-label">
                  <input id="verified1" type="radio" name="is_active" value="1" class="form-check-input" @if(old('is_active')) checked @endif/>
                  Активный
                  <i class="input-helper"></i>
                </label>
              </div>
