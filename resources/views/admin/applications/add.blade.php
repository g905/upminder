  ...
  <div class="btn-group">
    @foreach($applications_types as $type)
      <button type="button" data-card="{{ $type->short_code }}"
          class="btn btn-small btn-primary btn-tabs show_card {{ old('app_type') == $type->id || $loop->first ? 'active' : '' }}">
        {{ $type->name }}
      </button>
    @endforeach
  </div>
        ...
        @foreach($applications_types as $type)
          <div class="card-body dynamic {{ $type->short_code }}">
            ...
            @if($type->short_code === 'find')
              <form action="{{ route('admin.applications.store', ['app_type' => $type->id]) }}"
                  method="POST"
                  class="form_group_margin mentor_form">
                @csrf
                <div class="form-group">
                  <label>Почта: *</label>
                  <input type="text" name="email" value="{{ old('email', '') }}" placeholder="test@test.ru" class="form-control-sm form-control" />
                </div>
                <div class="form-group">
                  <label>Фамилия: *</label>
                  <input type="text" name="last_name" value="{{ old('last_name', '') }}" placeholder="Например: Иванов" class="form-control-sm form-control" />
                </div>
                <div class="form-group">
                  <label>Имя: *</label>
                  <input type="text" name="first_name" value="{{ old('first_name', '') }}" placeholder="" class="form-control-sm form-control" />
                </div>
                <div class="form-group">
                  <label>Отчество: *</label>
                  <input type="text" name="surname" value="{{ old('surname', '') }}" placeholder="" class="form-control-sm form-control" />
                </div>
                <div class="form-group">
                  <label>Телефон: *</label>
                ...
                <div class="form-group">
                  ...
                  <select name="communicate_method" size="7" class="select2 form-control-sm form-control">
                    @foreach(config('app.communicate_method') as $id => $type)
                      <option value="{{ $id }}" @if ($id == old('communicate_method','')) selected @endif>{{ $type }}</option>
                    @endforeach
                  </select>
                </div>
                <div class="form-group">
                  <label>Цель консультаци: </label>
                  <textarea name="purpose_mentoring" rows="8" class="visual form-control">{{ old('purpose_mentoring', '') }}</textarea>
                </div>
                <div class="form-group">
                  <label>Язык: </label>
                  <select name="language_id" size="7" class="select2 form-control-sm form-control">
                    @if ($languages->count())
                      @foreach ($languages as $language)
                        <option value="{{ $language->id }}" @if ($language->id == old('language_id','')) selected @endif>{{ $language->name }}</option>
                      @endforeach
                    @endif
                  </select>
                </div>
            ...
            @elseif($type->short_code === 'lesson')
              <form action="{{ route('admin.applications.store', ['app_type' => $type->id]) }}"
                    ...
                    @if ($mentors->count())
                      @foreach ($mentors as $mentor)
                        <option value="{{ $mentor->id }}"
                            @if ($mentor->id == old('mentor_id', '')) selected @endif>
                ...
                <div class="form-group">
                  <label>Почта: *</label>
                  <input type="text" name="email" value="{{ old('email','') }}" placeholder="" class="form-control-sm form-control" />
                </div>
                <div class="form-group">
                  <label>Фамилия: *</label>
                  <input type="text" name="last_name" value="{{ old('last_name', '') }}" placeholder="Например: Иванов" class="form-control-sm form-control" />
                      ...
                      @foreach ($mentors as $mentor)
                        <optgroup label="{{ $mentor->last_name }} {{ $mentor->first_name }} {{ $mentor->surname }}">
                          @foreach($mentor->tags as $tag)
                            <option value="{{ $tag->id }}" @if(in_array($tag->id, old('mentor_tag_id', []))  ) selected @endif>{{ $tag->name }}</option>
                          @endforeach
                        </optgroup>
                      @endforeach
                    @endif
                      ...
                      @foreach ($mentors as $mentor)
                        <optgroup label="{{ $mentor->last_name }} {{ $mentor->first_name }} {{ $mentor->surname }}">
                          @foreach($mentor->services as $item)
                            <option value="{{ $item->id }}" @if ($item->id == old('mentor_service_id','')) selected @endif>{{ $item->service }}</option>
                          @endforeach
                        </optgroup>
                      @endforeach
                    @endif
            ...
            @elseif($type->short_code ==='mentoring')
              <form action="{{ route('admin.applications.store', ['app_type' => $type->id]) }}"
                  method="POST" class="form_group_margin mentor_form"
                ...
                <div class="form-group">
                  ...
                  <input type="text" name="timezone" value="{{ old('timezone', '') }}" class="form-control" />
                </div>
                <div class="form-group">
                  <label>Ссылка на резюме: </label>
                  <input type="text" name="resume_link" value="{{ old('resume_link', '') }}" class="form-control" />
                </div>
                <div class="form-group">
                  <label>Резюме: </label>
            ...
            @elseif($type->short_code ==='company')
              <form action="{{ route('admin.applications.store', ['app_type' => $type->id]) }}"
                ...
                <div class="form-group">
                  ...
                  <input type="text" name="law_name" value="{{ old('law_name' ?? '') }}" class="form-control" />
                </div>
                <div class="form-group">
                  <label>Инн компании: </label>
                  <input type="text" name="inn" value="{{ old('inn' ?? '') }}" class="form-control" />
                </div>
