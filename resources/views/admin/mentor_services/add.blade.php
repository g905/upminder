...
@section('content')
  ...
  <div class="row">
    <div class="col-md-12 grid-margin">
      ...
      <div class="card">
        <div class="card-body">
          <form action="{{ route('admin.mentor_services.store') }}" method="POST">
            @csrf
              ...
              <select name="type_service" class="select2 form-control-sm form-control">
                ...
                @if (count($service_types))
                  @foreach ($service_types as $index => $item)
                    <option value="{{ $index  }}" @if ($index == old('type_service', '')) selected @endif>
                      @if($item == 'main')
                        Основная
                      @else
                        Дополнительная
            ...
            <div class="form-group">
              ...
              <input type="text" name="name" value="{{ old('name', '') }}" placeholder="Например: Иванов" class="form-control-sm form-control" />
            </div>
