                ...
                @if ($list->count())
                  @foreach ($list as $rec)
                    <tr>
                      ...
                      <td align="right">
                        <a href="{{ route('admin_task_type_form', $rec->id) }}" class="badge badge-info">
                          <i class="remove mdi mdi-pencil"></i></a>
                        <a href="{{ route('admin_delete_record', ['task_type', $rec->id]) }}" data-confirm="Удалить тип задач {{ $rec->name }}?" class="confirm badge badge-danger">
