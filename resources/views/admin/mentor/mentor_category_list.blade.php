			...
			<div class="card">
				<div class="card-body table">
                  ...
                  @foreach($parent_categories as $parent_cat)
                    <tr>
                      <td><input type="checkbox" name="ids[]" value="{{ $parent_cat->id }}" /></td>
                      <td>
                        <strong>
                          {{ $parent_cat->id }}
                        </strong>
                      </td>
                      <td>{!! $parent_cat->parent !!}</td>
                      <td>
                        <a class="badge badge-dark fs-6" href="{{ route('admin_mentor_cat_form', $parent_cat->id) }}">
                          <strong>
                            {{ $parent_cat->name }}
                          </strong>
                        </a>
                      </td>
                      <td align="center">{{ $parent_cat->mentors }}</td>
                      <td align="right">
                        <a href="{{ route('admin_mentor_cat_form', $parent_cat->id) }}" class="badge badge-info"><i class="mdi mdi-pencil btn-icon-append"></i></a>
                        <a href="{{ route('admin_delete_record', ['mentor_category', $parent_cat->id]) }}" data-confirm="Удалить город {{ $parent_cat->name }}?" class="confirm badge badge-danger"><i class="remove mdi mdi-close-circle-outline"></i></a>
                      </td>
                    </tr>
                    @foreach ($parent_cat->children as $rec)
                      @if($rec->parent_id == $parent_cat->id)
                      <tr>
                        <td><input type="checkbox" name="ids[]" value="{{ $rec->id }}" /></td>
                        <td>{{ $rec->id }}</td>
                        <td>{!! $rec->parent->name !!}</td>
