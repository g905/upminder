...
@section('content')
  <div class="page-header">
    ...
    <a href="{{ route('admin.mentor.index') }}" class="btn btn-dark">Вернуться к списку</a>
  </div>
  ...
  <div class="btn-group">
    ...
    <button type="button" data-card="services_additional" class="btn btn-small btn-primary btn-tabs show_card">Доп услуги</button>
  </div>
  <div class="row">
    <div class="col-md-12 grid-margin">
      <form action="{{ route('admin_mentor_form', $id) }}" method="POST" class="form_group_margin mentor_form" enctype="multipart/form-data">
        ...
        <div class="card">
          <div class="card-body dynamic general">
            <div class="form-group">
              ...
              <input type="file" name="avatar" value="{{ old('avatar', '') }}" class="form-control upload" />
              @if($rec->avatar)
                <img src="{{ route('get.avatar', $rec->avatar)  }}" alt="" width="150">
              @endif
            </div>
            <div class="form-group">
              ...
              <select name="categories[]" size="7" multiple class="select2 multiple form-control-sm form-control">
                @if ($list_categories->count())
                  @foreach ($list_categories as $rec_list)
                    <optgroup label="{{ $rec_list->name }}">
                      @foreach($rec_list->children as $cat)
                        <option value="{{ $cat->id }}" @if (in_array($cat->id, $cat_list)) selected @endif @if ($cat->id == old('parent_id', $rec->parent_id)) selected @endif>{{ $cat->name }}</option>
                      @endforeach
                    </optgroup>
                  @endforeach
                @endif
            ...
            <div class="form-group">
              ...
              <select name="tag_id[]" size="7" multiple class="select2 form-control-sm form-control multiple">
                @foreach($mentor_categories as $cat)
                  <optgroup label="{{ $cat->name }}" data-id="{{ $cat->id }}" class="optgroup-category">
                    @foreach($cat->tags as $tag)
                      <option value="{{ $tag->id }}" @if(in_array($tag->id, old('tag_id', $rec->tags->pluck('id')->toArray())) ) selected @endif>{{ $tag->name }}</option>
                    @endforeach
                  </optgroup>
                @endforeach
              </select>
            ...
            <div class="form-group">
              ...
              <div class="form-check">
                <label class="form-check-label">
                  <input @if (old('is_active', $rec->is_active) == 1) checked @endif id="is_active1" type="radio" name="is_active" value="1" class="form-check-input" />
                  Активирован
                  <i class="input-helper"></i>
                </label>
              </div>
              <div class="form-check">
                <label class="form-check-label">
                  <input @if (old('is_active', $rec->is_active) == 0) checked @endif id="is_active0" type="radio" name="is_active" value="0" class="form-check-input" />
                  Не активирован
                  <i class="input-helper"></i>
                </label>
              </div>
            ...
            <div id="education_tpl" class="row row_education0 row_education row_tpl">
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <a href="javascript:void(0);" data-row="row_education0" class="badge badge-danger delete_row btn_row">
            ...
            <div id="experience_tpl" class="row row_experience0 row_experience row_tpl">
                  ...
                  <select name="experience_company[]" class="s2 form-control">
					...
					@if ($list_companies)
						@foreach ($list_companies as $c)
							<option value="{{ $c->id }}">{{ $c->name }} ({{ $c->law_name }})</option>
						@endforeach
					@endif
				  </select>
                </div>
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <a href="javascript:void(0);" data-row="row_experience0" class="badge badge-danger delete_row btn_row">
                    <i class="remove mdi mdi-close-circle-outline"></i></a>
                </div>
              </div>
            </div>
                  ...
                  <div class="row row_experience{{ $kk + 1 }} row_experience">
                    ...
                    <div class="col-md-3">
                      <div class="form-group">
						...
						<select name="experience_company[]" class="s2 form-control">
							...
							@if ($list_companies)
								@foreach ($list_companies as $c)
									<option value="{{ $c->id }}" @if ($exp->company_id == $c->id) selected @endif>{{ $c->name }} ({{ $c->law_name }})</option>
								@endforeach
							@endif
						</select>
					  </div>
                    ...
                    <div class="col-md-3">
                      <div class="form-group">
                        ...
                        <a href="javascript:void(0);" data-row="row_experience{{ $kk + 1 }}" class="badge badge-danger delete_row btn_row">
                          <i class="remove mdi mdi-close-circle-outline"></i></a>
                      </div>
                    </div>
                  </div>
          ...
          <div class="card-body dynamic services">
            <h3 class="mb-3">Основные услуги</h3>
            ...
            <div id="service_tpl" class="row row_service0 row_service row_tpl">
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <select name="service_service[]" class="form-control-sm form-control">
                    @if ($mentor_main_services->count())
                      @foreach ($mentor_main_services as $rec_curr)
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <a href="javascript:void(0);" data-row="row_service0" class="badge badge-danger delete_row btn_row">
                    <i class="remove mdi mdi-close-circle-outline"></i></a>
                </div>
              </div>
            </div>
            @if ($id > 0)
              @if ($services_main_list->count())
                @foreach ($services_main_list as $kk => $serv)
                  <div class="row row_service{{ $kk + 1 }} row_service">
                    ...
                    <div class="col-md-3">
                      <div class="form-group">
                        ...
                        <select name="service_service[]" class="form-control-sm form-control">
                          @if ($mentor_main_services->count())
                            @foreach ($mentor_main_services as $rec_curr)
                              <option value="{{ $rec_curr->id }}" @if($serv->service_id == $rec_curr->id) selected @endif>{{ $rec_curr->name }}</option>
                            @endforeach
                          @endif
                        </select>
                    ...
                    <div class="col-md-3">
                      <div class="form-group">
                        ...
                        <a href="javascript:void(0);" data-row="row_service{{ $kk + 1 }}" class="badge badge-danger delete_row btn_row">
                          <i class="remove mdi mdi-close-circle-outline"></i></a>
                      </div>
                    </div>
                  </div>
          ...
          <div class="card-body dynamic services_additional">
            <h3 class="mb-3">Дополнительные услуги</h3>
            <a href="javascript:void(0);" data-tpl="additional_service_tpl" data-row="row_additional_service" class="add_row btn btn-warning">Добавить ещё</a>
            <br /><br />
            <div id="additional_service_tpl" class="row row_service0 row_additional_service row_tpl">
              <div class="col-md-3">
                <div class="form-group">
                  <label>Услуга: *</label>
                  <select name="additional_service_service[]" class="form-control-sm form-control">
                    @if ($mentor_additional_services->count())
                      @foreach ($mentor_additional_services as $rec_curr)
                        <option value="{{ $rec_curr->id }}">{{ $rec_curr->name }}</option>
                      @endforeach
                    @endif
                  </select>
              ...
              <div class="col-md-3">
                <div class="form-group">
                  <label>Валюта: *</label>
                  <select name="additional_service_currency_id[]" class="form-control-sm form-control">
                    @if ($list_currencies->count())
                      @foreach ($list_currencies as $rec_curr)
                        <option value="{{ $rec_curr->id }}">{{ $rec_curr->name }}</option>
                      @endforeach
              ...
              <div class="col-md-3">
                <div class="form-group">
                  <label>Цена в выбранной валюте: *</label>
                  <input type="text" name="additional_service_price[]" value="" class="form-control" />
                </div>
              </div>
              <div class="col-md-3">
                <div class="form-group">
                  <label>Скидка в %: *</label>
                  <input type="text" name="additional_service_discount[]" value="" class="form-control minus_button" />
                  <a href="javascript:void(0);" data-row="row_service0" class="badge badge-danger delete_row btn_row">
                    <i class="remove mdi mdi-close-circle-outline"></i></a>
                </div>
              </div>
            ...
            @if ($id > 0)
              @if ($services_additional_list->count())
                @foreach ($services_additional_list as $kk => $serv)
                  <div class="row row_additional_service{{ $kk + 1 }} row_additional_service">
                    ...
                    <div class="col-md-3">
                      <div class="form-group">
                        <label>Цена в выбранной валюте: *</label>
                        <input type="text" name="additional_service_price[]" value="{{ $serv->price }}" class="form-control" />
                      </div>
                    </div>
                    <div class="col-md-3">
                      <div class="form-group">
                        <label>Скидка в %: *</label>
                        <input type="text" name="additional_service_discount[]" value="{{ $serv->discount }}" class="form-control minus_button" />
                        <a href="javascript:void(0);" data-row="row_service{{ $kk + 1 }}" class="badge badge-danger delete_row btn_row">
                          <i class="remove mdi mdi-close-circle-outline"></i></a>
                      </div>
                    </div>
        ...
        <div class="form-group">
          ...
          <a href="{[ route('admin.mentor.index') }}" class="btn btn-warning">Выйти без сохранения</a>
        </div>
      </form>
    </div>
  </div>
  ...
  <script>
    $(document).ready(function () {
      ...
      $("select[name='categories[]']").on('change', function () {
        ...
        $.ajax({
          url: "{{ route('ajax.get.category.tags') }}",
          ...
          success: function (response) {
            ...
            {
              var $selected = [@foreach($rec->tags->pluck('id') as $tag){{ $tag }}@if(!$loop->last),@endif @endforeach];
              
              $("select[name='tag_id[]'] optgroup").remove();
              ...
              $.each(response, function (key, data) {
                ...
                var $optgroup = `<optgroup label="${data.name}"></optgroup>`;
                $("select[name='tag_id[]']").append($optgroup);
                
                $.each(data.tags, function (tag_index, tag) {
                  if ($(`select[name='tag_id[]'] optgroup[label='${data.name}']`).length > 0)
                  {
                   ...
                   // option.prop('selected',true);
                    ...
                    $(`select[name='tag_id[]'] optgroup[label='${data.name}']`).append(option);
                  }
                });
                //let $option_list = $optgroup.append($options);
