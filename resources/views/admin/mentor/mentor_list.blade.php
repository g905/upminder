...
@section('content')
  ...
  <div class="row">
    <div class="col-md-12 grid-margin">
      <div class="card">
        <form action="{{ route('admin.mentor.index') }}">
            ...
            <div class="row">
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <br />
                  <a href="{{ route('admin.mentor.index') }}">Сбросить фильтры</a>
                </div>
              </div>
              <div class="col-md-3">
                <div class="form-group">
  ...
  <div class="row">
    <div class="col-md-12 grid-margin">
      ...
      <div class="card">
        <div class="card-body table">
          <div class="table-responsive">
            <table class="table">
              <thead>
                <th width="36px"><input type="checkbox" name="select_all" class="select_all" /></th>
                <th width="36px">
                  ID
                  @if ($order_by == 'id')
                    @if ($order_by_asc == 'desc')
                      <a href="{{ route('admin.mentor.index', 'order_by=id&order_by_asc=asc') }}">&LeftDownVectorBar;</a>
                    @else
                      <a href="{{ route('admin.mentor.index', 'order_by=id&order_by_asc=desc') }}">&LeftUpVectorBar;</a>
                    @endif
                  @else
                    <a href="{{ route('admin.mentor.index', 'order_by=id&order_by_asc=desc') }}">&LeftUpVectorBar;</a>
                  @endif
                ...
                <th width="250px">
                  ...
                  @if ($order_by == 'last_name')
                    @if ($order_by_asc == 'desc')
                      <a href="{{ route('admin.mentor.index', 'order_by=last_name&order_by_asc=asc') }}">&LeftDownVectorBar;</a>
                    @else
                      <a href="{{ route('admin.mentor.index', 'order_by=last_name&order_by_asc=desc') }}">&LeftUpVectorBar;</a>
                    @endif
                  @else
                    <a href="{{ route('admin.mentor.index', 'order_by=last_name&order_by_asc=desc') }}">&LeftUpVectorBar;</a>
                  @endif
                ...
                <th align="center">
                  ...
                  @if ($order_by == 'verified')
                    @if ($order_by_asc == 'desc')
                      <a href="{{ route('admin.mentor.index', 'order_by=verified&order_by_asc=asc') }}">&LeftDownVectorBar;</a>
                    @else
                      <a href="{{ route('admin.mentor.index', 'order_by=verified&order_by_asc=desc') }}">&LeftUpVectorBar;</a>
                    @endif
                  @else
                    <a href="{{ route('admin.mentor.index', 'order_by=verified&order_by_asc=desc') }}">&LeftUpVectorBar;</a>
                  @endif
                ...
                @if ($list->count())
                  @foreach ($list as $rec)
                    ...
                    <tr>
                      ...
                      <td>{{ $rec->id }}</td>
                      <td>
                        <a href="{{ route('admin_mentor_form', $rec->id) }}">{{ $rec->last_name }} {{ $rec->first_name }} {{ $rec->surname }}</a>
                      </td>
                      <td>
                        <div class="overflow_">{!! $rec->mentor_categories !!}</div>
                      </td>
                      ...
                      <td align="right">
                        <a href="{{ route('admin_mentor_view', $rec->id) }}" class="badge badge-success">
                          <i class="mdi mdi-eye"></i></a>
                        <a href="{{ route('admin_mentor_form', $rec->id) }}" class="badge badge-info">
                          <i class="remove mdi mdi-pencil"></i></a>
                        <a href="{{ route('admin_delete_record', ['mentor', $rec->id]) }}" data-confirm="Удалить ментора {{ $rec->last_name }} {{ $rec->first_name }}?" class="confirm badge badge-danger">
      ...
      <form action="{{ route('admin.mentor.delete.many') }}" method="post" class="mt-3 delete-all-form">
        @csrf
        <button class="btn btn-danger btn-delete-all" >Удалить выбранные</button>
      </form>
    </div>
  </div>
@endsection
  ...
  <script>
    $(document).ready(function () {
      $(".btn-delete-all").on('click', function (e) {
        ...
        {
          let $ids = [];
          $(`.delete-all-form input[type='checkbox']`).remove();
          $("input[name='ids[]']:checked").each(function (i, el) {
            $ids.push(parseInt($(el).val()));
            let $id = parseInt($(el).val());
            if (!$(`.delete-all-form input[value='${$id}']`).length)
              $(".delete-all-form").append(`<input type="hidden" name="ids[]" value="${parseInt($(el).val())}" >`);
          })
