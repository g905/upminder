...
@section('content')
	<div class="page-header">
		...
		<a href="{{ route('admin_company_cat_form', 0) }}" class="btn btn-success">Добавить</a>
		@if ($filters['parent_id'] > 0)
			<a href="{{ route('admin_company_cat_form', 0) }}?parent_id={{ $filters['parent_id'] }}" class="btn btn-info">Добавить подкатегорию</a>
		@endif
	</div>
	<div class="btn-group">
	...
	<div class="row">
		<div class="col-md-12 grid-margin">
			<div class="card">
				<form action="{{ route('admin_company_cat_list') }}">
						...
						<table class="table">
							<thead>
								...
								<th width="36px">
									...
									@if ($order_by == 'id')
										@if ($order_by_asc == 'desc')
											<a href="{{ route('admin_company_cat_list', 'order_by=id&order_by_asc=asc') }}">&LeftDownVectorBar;</a>
										@else
											<a href="{{ route('admin_company_cat_list', 'order_by=id&order_by_asc=desc') }}">&LeftUpVectorBar;</a>
										@endif
									@else
										<a href="{{ route('admin_company_cat_list', 'order_by=id&order_by_asc=desc') }}">&LeftUpVectorBar;</a>
									@endif
								...
								<th>
									...
									@if ($order_by == 'name')
										@if ($order_by_asc == 'desc')
											<a href="{{ route('admin_company_cat_list', 'order_by=name&order_by_asc=asc') }}">&LeftDownVectorBar;</a>
										@else
											<a href="{{ route('admin_company_cat_list', 'order_by=name&order_by_asc=desc') }}">&LeftUpVectorBar;</a>
										@endif
									@else
										<a href="{{ route('admin_company_cat_list', 'order_by=name&order_by_asc=desc') }}">&LeftUpVectorBar;</a>
									@endif
								</th>
								<th width="50px" align="center">Кол-во компаний</th>
								...
								@if ($list->count())
									@foreach ($list as $rec)
										<tr>
											...
											<td>{{ $rec->id }}</td>
											<td>{!! $rec->parent !!}</td>
											<td><a href="{{ route('admin_company_cat_form', $rec->id) }}">{{ $rec->name }}</a></td>
											<td align="center">{{ $rec->companies }}</td>
											<td align="right">
												<a href="{{ route('admin_company_cat_form', $rec->id) }}" class="badge badge-info"><i class="mdi mdi-pencil btn-icon-append"></i></a>
												<a href="{{ route('admin_delete_record', ['company_category', $rec->id]) }}" data-confirm="Удалить город {{ $rec->name }}?" class="confirm badge badge-danger"><i class="remove mdi mdi-close-circle-outline"></i></a>
											</td>
										</tr>
									@endforeach
								@else
